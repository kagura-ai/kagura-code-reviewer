...
```bash
.venv/bin/pytest -q                                  # run the test suite (138 tests)
...
Release/version rules live in `.claude/rules/releasing.md` (imported below). In short:
the package version lives in **four** files that CI `version-sync` keeps in lock-step,
and `/gh-issue-driven:tag` only bumps two of them — bump all four (especially
`pyproject.toml` + `src/kagura_code_reviewer/__init__.py`) **before** tagging.

@.claude/rules/releasing.md
